...
-   **Labelling.** Can we create a solid figure caption and better axis labels for this figure? In your caption, please specify the task(s) the visualizaiton is intended to facilitate, as well as the marks, channels, and key-value pairs used.
    ...
    -   **Answer:**
        ...
        Task:To help users understand the participation of players in NHL games from different draft rounds.
...
-   **Key-Value pairs:** Looks like we are using "round" as a continuous variable. Can we change this to an ordered factor?
    ...
    -   Although "round" has a certain order, the "distance" between two "rounds" is not continuous. For example, the gap between the first round and the second round is not the same as the gap between the seventh round and the eighth round. Changing "round" to an ordered factor can more accurately reflect this relationship.
